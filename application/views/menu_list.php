...
<section class="inner-page">
	...
	<div class="container">
    	...
    	<div class="content-page register-now menu-page" id="ShowForm">
       	  ...
       	  <div class="fleft"><h2>Customer Feedback</h2></div>
          ...
          <input type="hidden" name="temporary" id="temporary" value="1">

          <input type="hidden" name="order_timestamp" id="order_timestamp" value="<?php echo $order_timestamp; ?>">

          <input type="hidden" name="redirect_file" id="redirect_file" value="order/menu_list/">
          ...
          <!--<select name="customerid" required id="customerid" style="width:200px;height:45px;padding:5px;font-size:20px;" >
          ...
          <div class="fleft order-summary">
            ...
            <div id="TakOrderError"></div> 

            <input type="submit" name="SubmitBtn" id="SubmitBtn" value="Take Feedback" class="Button" style="background-color:#C30003;" />

            </div>

            

            <div class="feedback-btn" id="start_feedback_button_div"><div id="start_feedback_button_error"></div><input type="button" name="start_feedback_button" id="start_feedback_button" value="Start Feedback  order(s)" class="Button" style="background-color:#C30003;font-size: 22px;" /></div>
...
function count_temporary_pending_orders(table_number)

{
	...
	$.ajax({
	  ...
	  url: WEB_URL+"order/count_temporary_pending_orders/",
	  ...
	  success: function(Data, textStatus){
...

		{
			...
			total_orders_pending = Data.total_orders_pending;
	...
	return total_orders_pending;

}

function check_temporary_pending_orders(table_number)

{

	var total_orders_pending = count_temporary_pending_orders(table_number);
...

	{
		...
		var order_string = total_orders_pending == 1 ? 'order' : 'orders';

		//$('#start_feedback_button_div').css('display', '');

		$('#start_feedback_button_div').show();
		$('#SubmitBtn').hide();

		//$('#start_feedback_button').val('Start Feedback '+total_orders_pending+' ' + order_string);
		$('#start_feedback_button').val('Take Feedback');
...

	{
		...
		$('#start_feedback_button_div').hide();

		//$('#start_feedback_button').val('Start Feedback '+total_orders_pending+' order');
		$('#start_feedback_button').val('Take Feedback');
...
check_temporary_pending_orders('<?php echo $Table ? $Table : 0; ?>');



// For temporary orders start feedback button

$(document).on('click', '#start_feedback_button', function(){

	var Form = $('#TakeOrderForm').serialize();

	$('#start_feedback_button_error').html('Please wait...');
	...
	$.post(WEB_URL+"order/make_feedback_for_single_table", Form, function(Data, textStatus){
		...
		if(Data.Error==0)
			...
			$('#start_feedback_button_error').html(Data.Msg);
